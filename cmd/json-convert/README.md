...
| --- | --- | --- |
| `--out must be json or json5` | `--out` was omitted or has an unsupported value. | Specify `--out json` or `--out json5`. |
...
| `expected exactly two file paths` | `INPUT` or `OUTPUT` is missing, or there are extra positional arguments. | Keep exactly two file paths. |
